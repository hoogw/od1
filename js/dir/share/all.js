...
//  - - -  ---  select model radio   ---  - - -
var selected_model = "esri" // default model set here
...
async function get_mapserver_info_html(mapserver_url){
 ...
 /**/
  ...
  function setRadioByValueLoop(name, valueToSelect) {
  const radios = document.getElementsByName(name);

  for (let i = 0; i < radios.length; i++) {
    if (radios[i].value === valueToSelect) {
      radios[i].checked = true;
      break; // Stop looping once the correct radio button is found
    }
  }
}
    ...
    function ui_event_register(){
        ...
        if ($("input[type=radio][name=model-radio]")){

            // first time set radio
            $("input[type=radio][name=model-radio][value=" + selected_model + "]").prop('checked', true);
            $("input[type='radio'][name='model-radio']").change(function(){
                        selected_model = $("input[type='radio'][name='model-radio']:checked").val();
                        console.log(" selected model changed to : --  ", selected_model);
                      });
        }//if
      

        /**/
        //  --- end  --- select model radio    - - -  --- 
        /**/
...
function use_what_model_to_open_popup(__model__name__, _name_, _type_, _url_, _id_){
    ...
    switch (__model__name__) {
        ...
        case 'apple':
            ...
            _newTab_link =  url_template_apple
            _newTab_link += '?layer_id=' + _id_  + '&url=' + _url_ 
            break;    


        default:
            // always use basic model 
            _newTab_link =  url_template_esri
            _newTab_link += '?backgroundlayerurl=' + _url_ + "/" + _id_
